...
def true_correct(output, problem: Problem=None):
    ...
    if isinstance(output, list):
        ...
        if gpt_sol is None or len(gpt_sol) == 0:
        ...
        my_print(f"Error: output should be either list or str type.")
        correct = False
        di_correct = False
        gpt_sol = ""
    ...
    if not parser.parsed:
        di_correct = False
        my_print(gpt_sol)
        my_print("Could fail by the following reasons:")
        ...
        if not parser.sentence_lst:
            my_print("Not a single valid sentence is found")
        correct = False
    else:
        parser.parse(problem=problem)
    if parser and parser.sentence_lst:
    ...
    if parser.incorrect_lst:
        ...
        my_print(f"Mistake happened on param {problem.get_ntn(param=param)}")
        ...
        correct = False
        di_correct = False    
    if correct and parser.sentence_lst:
        if parser.sentence_lst[-1].param_part != problem.get_ntn(problem.ques_idx):
            ...
            for sol_step in parser.sol_steps:
                print(sol_step)
    return di_correct, correct, my_print, parser
